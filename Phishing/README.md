...
В этой лабораторной работе мы будем использовать инструмент [Gophish](https://getgophish.com/) для проведения фишинговой рассылки. Для этого мы поднимем свой почтовый сервер и будем экспериментировать на нём. Напоминаю, что проведение подобной атаки без письменного разрешения является преступлением, поэтому мы сами создадим себе пользователей и наши фишиговые письма распространятся только внутри нашей локальной сети. 
...
```
Почтовый сервер должен сообщить об ошибках, так как у нас нет пользователей и нет подписанного сертификата.
...
3. Создаём новый пароль
4. Настраиваем профиль отправителя(**Sending Profile**)
